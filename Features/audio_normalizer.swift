...
class AudioNormalizer {
    ...
    private static func normalizeAudio(
        ...
        if let systemPath = try? await findSystemFFmpeg() {
            ffmpegPath = systemPath
            statusCallback("Normalizing audio using system ffmpeg")
            print("✅ Using system ffmpeg: \(systemPath)")
        } else if let bundledPath = BundleHelpers.ffmpegPath {
            ffmpegPath = bundledPath
            statusCallback("Normalizing audio using bundled ffmpeg (install Homebrew ffmpeg for faster processing)")
            print("🔧 Using bundled ffmpeg: \(bundledPath)")
        } else {
            throw DownloaderError.missingBinary("ffmpeg not found")
        }
    ...
    private static func findSystemFFmpeg() async throws -> String? {
        let candidates = [
        ...
        print("🔍 Searching for system ffmpeg...")
        ...
        for path in candidates {
            ...
            do {
                ...
                if process.terminationStatus == 0 {
                    ...
                    if let output = String(data: data, encoding: .utf8), output.contains("ffmpeg") {
                        print("✅ System ffmpeg verified: \(path)")
                        return path
                    }
                }
            } catch {
                print("❌ Cannot execute \(path): \(error)")
                continue
            }
        }
        
        print("❌ No usable system ffmpeg found")
        return nil
    }
}
